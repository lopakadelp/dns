...
provides :dns_dnsmadeeasy_api20
...
action :create do
  ...
  args = {
    'name' => subdomain.to_s,
    'type' => new_resource.type.upcase.to_s,
    'value' => new_resource.entry_value.to_s,
...
def auth_headers
  ...
  missing_args = []
  %w(dnsmadeeasy_api_key dnsmadeeasy_secret_key).each do |auth_key|
    unless new_resource.credentials.key?(auth_key) && !new_resource.credentials[auth_key].to_s.empty?
  ...
  {
    :"x-dnsme-apiKey" => api_key.to_s,
    :"x-dnsme-hmac" => hmac.to_s,
    :"x-dnsme-requestDate" => request_time.to_s
  }
end
