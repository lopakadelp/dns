...
describe 'Run DNSMadeEasy provider' do
  ...
  let(:chef_run) do
    runner = ChefSpec::SoloRunner.new(step_into: ['dns'])
    runner.node.override[:dns][:credentials] = { dnsmadeeasy_api_key: 'key', dnsmadeeasy_secret_key: 'secret' }
    runner.node.override[:dns][:disable] = false
    runner.node.override[:dns][:domain] = domain
    runner.node.override[:dns][:entry][:name] = name
    runner.node.override[:dns][:entry][:value] = initial_value
    runner.node.override[:dns][:entry][:type] = type
    runner.node.override[:dns][:entry][:ttl] = ttl
  ...
  let(:chef_run_dns_disable) do
    runner = ChefSpec::SoloRunner.new(step_into: ['dns'])
    runner.node.override[:dns][:credentials] = { dnsmadeeasy_api_key: 'key', dnsmadeeasy_secret_key: 'secret' }
    runner.node.override[:dns][:disable] = true
    runner.node.override[:dns][:domain] = domain
    runner.node.override[:dns][:entry][:name] = name
    runner.node.override[:dns][:entry][:value] = initial_value
    runner.node.override[:dns][:entry][:type] = type
  ...
  it 'creates a new record using dnsmadeeasy v2.0' do
    expect(rest_client).to receive(:get).exactly(2).and_return(
      %({"data":[{"name":"#{domain}","id":123456,"created":1192147200000}]}),
      %({"data":[]})
    )
  ...
  it 'updates an existing record using dnsmadeeasy v2.0' do
    expect(rest_client).to receive(:get).exactly(2).and_return(
      %({"data":[{"name":"#{domain}","id":123456,"created":1192147200000}]}),
      %({"data":[{"name":"#{name}","value":"#{updated_value}","id":12345678,"type":"#{type}","ttl":#{ttl}}]})
    )
  ...
  it 'does no update to an existing record using dnsmadeeasy v2.0' do
    expect(rest_client).to receive(:get).exactly(2).and_return(
      %({"data":[{"name":"#{domain}","id":123456,"created":1192147200000}]}),
      %({"data":[{"name":"#{name}","value":"#{initial_value}","id":12345678,"type":"#{type}","ttl":#{ttl}}]})
    )
  ...
  it 'deletes a record using using dnsmadeeasy v2.0' do
    expect(rest_client).to receive(:get).exactly(2).and_return(
      %({"data":[{"name":"#{domain}","id":123456,"created":1192147200000}]}),
      %({"data":[{"name":"#{name}","value":"192.168.1.2","id":12345678,"type":"#{type}","ttl":#{ttl}}]})
    )
